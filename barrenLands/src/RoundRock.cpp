...
void RoundRock::subdivideFace(std::vector<glimac::ShapeVertex> &_vertices) { //prend un vecteur de 3 vertices
    ...
    subDiv1 = (vertices[1].position - _vertices[0].position)/2 + vertices[0].position;
    subDiv2 = (vertices[2].position - _vertices[1].position)/2 + vertices[1].position;
    subDiv3 = (vertices[0].position - _vertices[2].position)/2 + vertices[2].position;

    glimac::ShapeVertex v1(glm::vec3(glm::normalize(subDiv1)),
                           glm::vec3(glm::normalize(subDiv1)),
                           glm::vec2(1,1)
    );

    glimac::ShapeVertex v2(glm::vec3(glm::normalize(subDiv2)),
                           glm::vec3(glm::normalize(subDiv2)),
                           glm::vec2(1,1)
    );

    glimac::ShapeVertex v3(glm::vec3(glm::normalize(subDiv3)),
