...
PreciousRock::PreciousRock() : ProceduralRock(){
    generateVertices();
    generateIndices();
...
void PreciousRock::generateVertices(){
...
void PreciousRock::generateIndices(){
    indices.clear();
}
void PreciousRock::generateNormals() {
    ProceduralRock::generateNormals();
}


void PreciousRock::subdivideObject(std::vector<glimac::ShapeVertex> &_vertices, int nbRecurse) {
    if(nbRecurse <= 0){
        return;
    }
...
Color *PreciousRock::chooseColor(Color *_color) {
    Color * alteredColor;
    if(_color == nullptr){
...
int PreciousRock::find(std::vector<glm::vec3> &tab, glm::vec3 object){
    //Chercher un objet
    int i;
...
void PreciousRock::smooth(std::vector<glimac::ShapeVertex> &_vertices, int nbRecurse) {
    if(nbRecurse <= 0){
        return;
    }
