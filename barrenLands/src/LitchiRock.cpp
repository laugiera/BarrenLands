...
LitchiRock::LitchiRock() : ProceduralRock(){
    generateVertices();
    generateIndices();
...
LitchiRock::~LitchiRock(){}
void LitchiRock::generateVertices(){
...
void LitchiRock::generateIndices(){
    indices.clear();
}
void LitchiRock::generateNormals() {
    ProceduralRock::generateNormals();
}


void LitchiRock::subdivideObject(std::vector<glimac::ShapeVertex> &_vertices, int nbRecurse) {
    if(nbRecurse <= 0){
        return;
    }
...
Color *LitchiRock::chooseColor(Color *_color) {
    Color * alteredColor;
    if(_color == nullptr){
...
int LitchiRock::find(std::vector<glm::vec3> &tab, glm::vec3 object){
    //Chercher un objet
    int i;
...
void LitchiRock::smooth(std::vector<glimac::ShapeVertex> &_vertices, int nbRecurse) {
    if(nbRecurse <= 0){
        return;
    }
...
glm::mat4 LitchiRock::getRandomScale() {
    return glm::scale(glm::mat4(1.f), glm::vec3(NoiseManager::getInstance().getRandomFloat()/3.5));
}
