...
TriangularRock::TriangularRock() : ProceduralRock(){
    generateVertices();
    generateIndices();
...
TriangularRock::~TriangularRock(){}
void TriangularRock::generateVertices(){
...
*/
    ...
    _vertices.emplace_back(glm::vec3(cos(2*3.14/3),-1,sin(2*3.14/3)),glm::normalize(glm::vec3(cos(2*3.14/3),-1,sin(2*3.14/3))), glm::vec2(1,1));
    _vertices.emplace_back(glm::vec3(cos(-2*3.14/3),-1,sin(-2*3.14/3)), glm::normalize(glm::vec3(cos(-2*3.14/3),-1,sin(-2*3.14/3))), glm::vec2(1,1));
    _vertices.emplace_back(glm::vec3(0, 1, 0), glm::normalize(glm::vec3(0, 1, 0)), glm::vec2(1,1));
...
void TriangularRock::generateIndices(){
    indices.clear();
}
void TriangularRock::generateNormals() {
    ProceduralRock::generateNormals();
}


void TriangularRock::subdivideObject(std::vector<glimac::ShapeVertex> &_vertices, int nbRecurse) {
    if(nbRecurse <= 0){
        return;
    }
...
Color *TriangularRock::chooseColor(Color *_color) {
    Color * alteredColor;
    if(_color == nullptr){
...
int TriangularRock::find(std::vector<glm::vec3> &tab, glm::vec3 object){
    //Chercher un objet
    int i;
...
void TriangularRock::smooth(std::vector<glimac::ShapeVertex> &_vertices, int nbRecurse) {
    if(nbRecurse <= 0){
        return;
    }
